...
from geouned.geometry_backend.geometry_backend_interface import GBoundBox, GCylinder, GPlane, GVector
...
def test_is_in_plane():
    plane = GPlane(Position=GVector(0, 0, 0), Axis=GVector(0, 0, 1))
...
def test_plane_value_at():
    plane = GPlane(Position=GVector(1, 2, 3), Axis=GVector(0, 0, 1), XDir=GVector(1, 0, 0))
    assert plane_value_at(plane, 2, 5).is_equal(GVector(3, 7, 3), 1e-9)


def test_plane_tangent_at():
    plane = GPlane(Position=GVector(0, 0, 0), Axis=GVector(0, 0, 1), XDir=GVector(1, 0, 0))
...
def test_cylinder_value_at_on_axis():
    cylinder = GCylinder(Center=GVector(0, 0, 0), Axis=GVector(0, 0, 1), Radius=2.0, XDir=GVector(1, 0, 0))
...
def test_cylinder_value_at_without_x_dir_raises():
    cylinder = GCylinder(Center=GVector(0, 0, 0), Axis=GVector(0, 0, 1), Radius=2.0)
    with pytest.raises(ValueError):
