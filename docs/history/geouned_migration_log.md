# GEOUNED migration — detailed session-by-session log

This file is the full chronological investigation log for the FreeCAD -> pyOCC
migration: every bug found and fixed, the methodology used to verify each fix
(usually a d1suned MCNP stochastic volume check, a `Solidos/` corpus
differential scan, or point-sampling against real CAD ground truth), and the
hard-won lessons about where a fix can silently regress an unrelated file.

It used to live inline in `CLAUDE.md` (which had grown past 12,000 lines);
it was split out here on 2026-09-12 to keep `CLAUDE.md` itself a short,
current-state summary. Nothing was rewritten or summarized in the split —
every section below is unchanged from where it was written, in the order it
was written, in the original author's own voice (`assistant` narrating each
investigation as it happened, not `we`/`you`).

**Read `CLAUDE.md` first** for the project overview, current architecture, and
current known-open items — come here only when you need the full story behind
a specific fix, a debugging technique that worked before, or the reasoning
that a shortcut was already tried and rejected.

This file is read chronologically — entries near the bottom are the most
recent and, where they say a section above them is "closed"/"superseded"/
"resolved", they take precedence over what that earlier section says.
...
## Known-open-items audit, 2026-09-12: `modelcell_cut1.stp`/`modelCell_670000.stp`
confirmed fixed, 5 other items confirmed still open

User request, right after the CLAUDE.md reorganization above: verify the
new "Known open items" list is actually still accurate rather than
trusting it copied forward correctly. Checked each item against the
current code/tests, not just re-read the prose:

- **`AdjacentMultiplanePlanes`/MultiRoundCorner** — still open. Grepped
  the code: `AdjacentMultiplanePlanes` is only read/written on the RevCC
  path (`geouned_classes.py:1276`, `basic_functions_part1.py:623-630`);
  `add_multiRoundCorner`/`multi_round_corner_region` have no equivalent.
- **`Big_complex_cell/modelcell_cut1.stp` and `modelCell_670000.stp`** —
  **no longer open.** Converted both with `verify_one_solid.py` (default
  `Options()`, i.e. `cut_large_cell` at its effectively-disabled
  default, `forceNoOverlap=False`) under `ocp`, then ran a real d1suned
  check: `modelcell_cut1.stp` converts in 23.7s (was
  crash/STACK_OVERFLOW-prone), tally `0.99642 +/- 0.19%` (1.9 sigma),
  0 lost particles; `modelCell_670000.stp` converts in 253s (matching
  its own historically-documented "slow, ~230s"), tally `0.99825 +/-
  0.18%` (1.0 sigma), 0 lost particles. Neither file's own `Options`
  needed `cut_large_cell` enabled to get this result — the fix that
  made this possible is almost certainly `generic_split`'s new
  post-split volume-conservation guard (added in the `large_cell_plane_split`
  commit while investigating `modelCell_670000.stp` specifically, but
  applied unconditionally to every candidate split, not gated behind
  `cut_large_cell`) — consistent with that commit's own account of a
  MultiPlane candidate silently under-separating a fragment of this
  exact file.
- **`hylife-v06.stp` solid 45's slow decomposition** — still open;
  grepped the log for any later mention, none found since the entry
  that first documented it.
- **The 6 exotic quadric surfaces in `GEOReverse`** — still open;
  confirmed all 7 `Gmake_*` names in both `_occ_impl.py`/`_ocp_impl.py`
  still resolve to `_not_implemented(name)`, raising `NotImplementedError`.
- **`Solidos/` STEP fixture duplicates** — still open; an MD5 hash scan
  of the whole `Solidos/` tree found several still-duplicated files
  (`Cans/barrel_right.stp` == `working_solids/barrel_right.stp`,
  `Mixed/rev_pipe.stp` == `RoundCorners/rev_pipe.stp`,
  `RevCC_corpus_scan/*` == `test_models/RevCC_regression/*`, and more).
- **`build_region`/`GEOReverse` duplication** — still open; confirmed
  `src/geouned/GEOUNED/utils/build_region/` and
  `src/geouned/GEOReverse/Modules/{buildSolidCell,splitFunction}.py`
  remain separate files; only `Gfuse_solids` was ever consolidated into
  `geo/solid_ops.py`.
- **`test_cylbox_convertion`** — still open; ran
  `pytest tests/test_csgtocad.py -v` under `GEOUNED_CAD_ENGINE=ocp`
  directly: both `[mcnp]` (`assert 2 == 4`) and `[openmc_xml]`
  (`assert 1 == 5`) still fail with the same cell-count mismatch
  documented when this item was first opened.

CLAUDE.md's "Known open items" list updated accordingly (the
