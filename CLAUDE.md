...
`SCDR_90.stp` and `SCDR_90_piece0_badvolume.stp` moved to `Solidos/BadCADModel/` (genuine CAD-topology tangency issue, not a GEOUNED classification bug -- matches the established convention for that folder). `SCDR_90_piece1_badvolume.stp` (the worse of the two bad pieces, 81.6 sigma) remains in `convierte_bad_volume/`, not yet investigated -- likely worth checking for the same tangent-cut pattern first, given the structural similarity.
