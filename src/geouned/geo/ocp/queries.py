...
def Gsolid_max_tolerance(solid: GShape) -> float:
    ...
    while exp.More():
        worst = max(worst, BRep_Tool.Tolerance_s(TopoDS.Edge(exp.Current())))
        exp.Next()
    exp = TopExp_Explorer(native, TopAbs_VERTEX)
    while exp.More():
        worst = max(worst, BRep_Tool.Tolerance_s(TopoDS.Vertex(exp.Current())))
        exp.Next()
    return worst
...
def Gsolid_nonmanifold_edge_count(solid: GShape) -> int:
    ...
    TopExp.MapShapesAndAncestors_s(solid.__native__, TopAbs_EDGE, TopAbs_FACE, edge_faces)
    return sum(
        1
