...
from OCC.Core.gp import gp_Ax1, gp_Ax2, gp_Ax3, gp_Circ, gp_Dir, gp_Dir2d, gp_Pln, gp_Pnt, gp_Pnt2d, gp_Trsf, gp_Vec
...
"""Gsliver_heal's volume net -- looser than Gcollapse_split_rings' 3e-4
-- the sole numeric gate for Gsliver_heal (no count_split_ring_pairs
check: its planar cap is a thin annulus that metric false-counts). See
_ocp_impl.py's own identical constant: with _retrim_freed_quadrics a
correct heal conserves volume to ~1e-6 (LR.stp: dV 8.6e-7, d1suned tally
0.9985 / 0 lost, vs the input's tally 0.0 / 24 lost)."""
...
def _split_face_at_v_line(native_face, native_cone_surf, point_a: GVector, point_b: GVector) -> list:
    ...
    if u_hi - u_lo < 1e-9:
        ...
        # a construction error otherwise (confirmed live under OCP,
        # 2026-08-23, once the cone/cylinder candidate search started
        # reaching this surface; ported here for the same reason). Not a
        # real arc to split at; treat it the same as any other candidate
...
def _try_coaxial_cone_split(base: "GSolid", tool: "GSolid", tolerance: float) -> "list[GSolid] | None":
    ...
    *normal* BOP split on the resulting solid: once the arc already exists
    ...
    boundary. Ported from the `ocp` engine (confirmed live there,
    2026-08-23) alongside `ocp`'s own real fix for this function: the
    retry now escalates its own fuzzy tolerance (see below) rather than
    trying only the caller's `tolerance` once.
    """
    ...
    for other_face, mid_point, radius in candidates:
        ...
        # See _ocp_impl.py's own copy of this loop for the full account of
        # why a single retry at the caller's own (often near-zero)
        # `tolerance` isn't enough: a presplit's new edge is only exact to
        # floating-point precision, not identical to the tool's own BRep
        # representation of the same curve. Confirmed live (2026-08-23,
        # on the `ocp` engine against the same real fixture that motivated
        # this whole port): tolerance=0 through 0.05 all leave the
        # presplit unseparated, tolerance=0.1 splits it cleanly. The
