...
from ..utils.multi_planes import other_face_edge
...
def get_surfaces(solid, options, tolerances, numeric_format):
    ...
    kind_order = ("MultiP", "Planes", "Cyl", "Cone", "Sph", "Tor")
...
def cyl_bound_planes(solid, face):
    ...
    for e in Edges:
        ...
        if curve[0:6] == "Circle":
            ...
            dim1 = e.Curve.Radius
            dim2 = e.Curve.Radius
            plane = UF.GeounedSurface(("Plane", (center, dir, dim1, dim2)))
            planes.append(plane)
    ...
    for e in Edges:
        ...
        if curve[0:6] == "Circle":
            ...
            if planeParams is not None:
                plane = UF.GeounedSurface(("Plane", planeParams))
                planes.append(plane)
...
def extract_surfaces(solid, kind, options, tolerances, numeric_format):
    ...
    if kind == "All":
        ...
        if kind == "Plane3Pts":
            ...
            P3P = False
        solid_GU = GU.SolidGu(solid, tolerances=tolerances, plane3Pts=P3P)

    Surfaces = UF.MetaSurfacesDict(options=options, tolerances=tolerances, numeric_format=numeric_format)

    multiplanes_list, multiplane_faces = UF.get_multiplanes(solid_GU)
    ...
    for iface, face in enumerate(solid_GU.Faces):
        ...
        if surf == "<Plane object>" and kind in ["Planes", "All"]:
            ...
            plane = UF.GeounedSurface(("Plane", (pos, normal, dim1, dim2)))
            ...
            if kind in ["Cyl", "All"]:
                cylinder = UF.GeounedSurface(("Cylinder", (orig, dir, rad, dim_l)))
            ...
            if kind in ["Cone", "All"]:
                cone = UF.GeounedSurface(("Cone", (apex, dir, half_angle, dim_l, dimR)))
            ...
            sphere = UF.GeounedSurface(("Sphere", (pnt, rad)))
            ...
            if kind in ["Tor", "All"]:
                ...
                dir = face.Surface.Axis
                torus = UF.GeounedSurface(("Torus", (center, dir, radMaj, radMin)))
            ...
            plane = UF.GeounedSurface(("Plane3Pts", (pos, normal, dim1, dim2, points)))
            Surfaces.add_plane(plane, fuzzy)
